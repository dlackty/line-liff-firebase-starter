...
async function verifyLineToken(access_token: string) {
  const resp = await fetch(`https://api.line.me/oauth2/v2.1/verify?access_token=${access_token}`);
  const jsonResponse = await resp.json();
  if (jsonResponse.client_id !== myLineChannelId) {
    throw new Error(`LINE channel ID mismatched ${JSON.stringify(jsonResponse)}`);
  }
  const profileResponse = await fetch("https://api.line.me/v2/profile", { headers: { "Authorization": `Bearer ${access_token}` } });
